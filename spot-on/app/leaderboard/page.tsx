...
function getAvatarForUser(email: string): string {
  const numberPart = email.trim().split("@")[0].slice(1);
  const convertion = parseInt(numberPart);
  const mod = convertion % AVATARS.length;
              ...
              return (
                <div key={entry.id} className={`${rowBase} ${rowColor}`}>
                  ...
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-gray-700">
                    <Image
                      src={entry.image ?? getAvatarForUser(entry.email)}
                      alt={displayName}
                      width={40}
                      height={40}
