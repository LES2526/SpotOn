...
export function calculateCheckoutPoints(startTime: Date, spaceType: string, occupancy: number, capacity: number): number {
...
export const removeParticipantOp = (userId: string, sessionId: string) =>
    prisma.userOnStudySession.delete({ where: { userId_sessionId: { userId, sessionId } } });
