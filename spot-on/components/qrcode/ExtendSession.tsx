...
export default function ExtendStatus({ spaceId, isJoin }: Readonly<SuccessStatusProps>) {
    ...
    const handleConfirmDuration = async () => {
        try {
            if (!isJoin) {
                ...
                await axios.patch(`/api/spaces/${spaceId}/sessions/extend`, { expectedEndTime });
            }
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-950 text-white px-4">
            <div className="flex flex-col items-center gap-6 rounded-2xl border border-gray-800 bg-gray-900 p-10 shadow-xl max-w-sm w-full">
                ...
                <div className="text-center">
                        <h1 className="text-xl font-semibold text-white mb-2">{isJoin ? 'Joined Session!' : 'Extend Session'}</h1>
                        <p className="text-sm text-gray-500">{isJoin ? 'You have successfully joined the session.' : 'Select how much longer you want to extend the current session.'}</p>
                </div>
                ...
                >
                    {isJoin ? 'Go to Dashboard' : `Extend — ${formatDuration(durationMinutes)}`}
                </button>
            </div>
        </div>
    );
