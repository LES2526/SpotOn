...
A execução local recorre a \textbf{Docker Compose}, com dois serviços
declarados em \texttt{spot-on/compose.yaml}: a aplicação Next.js (construída
a partir de \texttt{spot-on/Dockerfile}) e a instância de PostgreSQL. Após
clonar o repositório, a aplicação é colocada em execução com um único
comando:
...
O \emph{schema} da base de dados é aplicado no arranque do \emph{container}
e o \emph{seed} é executado para popular as plantas, espaços e dados de
demonstração. Importa sublinhar que o \textbf{mesmo \texttt{Dockerfile}} é
reutilizado em produção pelo Railway (ver \S\ref{sec:railway} e
\S\ref{sec:prod}), garantindo paridade total entre os dois ambientes.
