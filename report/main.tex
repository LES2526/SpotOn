...
% =====================================================
\section{Arquitetura e Stack Tecnológica}
...
O diagrama de componentes (Figura~\ref{fig:arch}) é mantido em formato
\emph{drawio} no repositório
(\texttt{diagrams/component/backend\_arquitecture.drawio}) e exportado como
PNG para inclusão neste relatório.

\begin{figure}[H]
    \centering
    % TODO: substituir pela exportação final do diagrama de arquitetura
    % \includegraphics[width=0.85\linewidth]{figures/architecture_diagram.png}
    \fbox{\parbox[c][6.5cm][c]{0.85\linewidth}{%
            \centering\itshape Diagrama de componentes (em desenvolvimento)\\[0.4em]
            \small Substituir por \texttt{figures/architecture\_diagram.png}\\
            (fonte: \texttt{diagrams/component/backend\_arquitecture.drawio})}}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{@{}lll>{\raggedright\arraybackslash}X@{}}
        ...
        \midrule
        \emph{Framework} \emph{full-stack}           & Next.js (App Router, React)     & React + Express separados           & Unifica UI e API num só projeto, simplifica \emph{deployment} e tipagem partilhada.                                   \\
        ...
        Base de dados                                & PostgreSQL                      & MySQL, MongoDB                      & Modelo relacional adequado às entidades; \texttt{enum types} nativos; maturidade.                                     \\
        ...
        Envio de email                               & Nodemailer / Resend             & SMTP direto, AWS SES                & API simples para envio transacional dos \emph{magic links}.                                                           \\
        Containerização                              & Docker + Docker Compose         & VM, \emph{deploy} direto            & \emph{Setup} reprodutível em qualquer máquina com um único comando.                                                   \\
        Testes                                       & Jest + Postman/Newman           & Vitest, Mocha, Playwright           & Jest para unidade/integração; Newman para testes \emph{end-to-end} da API.                                            \\
        ...
        \bottomrule
    \end{tabularx}
    \caption{Stack tecnológica e alternativas consideradas.}
\end{table}
...
O diagrama UML de classes do domínio (Figura~\ref{fig:classdiag}) é mantido
em formato \emph{drawio} no repositório, em conjunto com a evolução do
\emph{schema} Prisma.

\begin{figure}[H]
    \centering
    % TODO: substituir pela exportação final do diagrama de classes
    % \includegraphics[width=0.95\linewidth]{figures/class_diagram.png}
    \fbox{\parbox[c][7cm][c]{0.85\linewidth}{%
            \centering\itshape Diagrama UML de classes do domínio (em desenvolvimento)\\[0.4em]
            \small Substituir por \texttt{figures/class\_diagram.png}\\
            (fonte: \texttt{diagrams/class/domain\_class\_diagram.drawio})}}
